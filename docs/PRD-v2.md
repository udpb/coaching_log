...
> **버전**: 2.1 — 2026-06-20 현행화 (최초 2.0 = 2026-06-04). 코드 정독(file:line 근거).
> **상태**: Living document. 코드/마이그레이션이 진실원본(SoT).
> **자매 PRD**: coach-finder `docs/PRD-v2.md`.
> **참고**: 본 레포가 **공유 Supabase 스키마의 SoT**(`supabase/migrations/`).
> **v2.1 반영**: ADR-023(status 4값 단일 라이프사이클·coach-finder=SoT) · ADR-022+K1b(프로젝트별 required_kpis·
> 추출 한국어 지표명) · ADR-021(2시간 청크 녹음·전사) · ADR-020(field-defs) · extraction 메타. [glossary](glossary.md) 참조.
...
**coaching-log 가 하는 것**: 코칭 세션 기록(STT/음성→Gemini 구조화), 팀별 성과 대시보드, 코치 본인 프로필·계약정보 자가관리, 사업기획(business_plans)·코치배정·평가(admin·pm), **사업 `active` 전환(=수주) 시 프로젝트 생성 트리거 지점**.
...
[form 세션 기록] ── 코칭 후, 말한 내용을 입력
  │
  ├─ 모드 A: STT transcript 붙여넣기 (≥50자) / 짧은 메모(20~49자, memo 모드)
  └─ 모드 B: 🎙 음성 녹음 (MediaRecorder) — 최대 2시간, 5분 청크 자동 분할 전사(ADR-021)
  │
...
[plans 사업기획] ── business_plans 생성 (admin/pm)
  ...
  │
  ▼
사업 진행 전환 (status='active' = 수주; coach-finder 가 SoT)
  │
  ▼  [DB 트리거 bp_lifecycle_sync_ins/_upd]
projects 자동 생성 + accepted 코치를 project_members 로 등록
  (종료/취소 시 bp_status_propagate_upd 가 projects.status 동기화)
...
**입력 모드**: ① 텍스트 STT 붙여넣기(≥50자) · ② 짧은 메모(20~49자 = `inputMode:'memo'`, 보수적 추출) · ③ 음성 녹음(webm/opus·mp4/aac). 단발 ~7분/4MB **또는 최대 2시간**(클라가 5분 청크로 분할 → `task:'transcribe'`(flash) 누적 전사 → 합본 추출, ADR-021).
...
**저장**: `coaching_logs` — 22필드 + narrative + evidence(jsonb) + (옵트인) 원본 transcript + metrics + coach_id + project_id + **추출 메타**(`extraction_model`·`extraction_version` — 프롬프트/모델 버전 감사, phase_aa).

> **필드 정의 SoT** = `public/field-defs.js`(ADR-020). 서버 프롬프트의 VALUE RULES·클라 폼 앵커·스트리밍 파서가
> 모두 이 한 파일에서 생성된다. 22필드 규칙 변경 시 `EXTRACTION_VERSION` 범프.
...
> 성과 지표 핵심 = `last_done_rate`(이행률) · `evidence`(필드별 근거) · `energy`(1-5) · `metrics`(지표).

**프로젝트별 필수 KPI (ADR-022)**: admin/pm 이 프로젝트마다 `projects.required_kpis`(jsonb `[{name}]`)를 지정 →
신규 일지 폼이 그 KPI 카드로 prefill. **추출 한국어 매칭(K1b)**: AI 가 지표 이름을 한국어로 반환하고 required_kpis
이름을 verbatim 재사용 → 영어 라벨/중복 카드 제거(`EXTRACTION_VERSION` 2026-06-20.1). 미설정 프로젝트는 전역 기본 메트릭 폴백.

### 4.3 사업/프로젝트 관리 (admin·pm)

- **business_plans**: 사업기획 단위. 상태 **`planning`→`active`→`completed`/`cancelled`** (4값 단일 라이프사이클, **coach-finder 가 SoT**, ADR-023). 구 draft/proposed/won/lost 폐지·흡수
- **수주 = `active` 전환**: coach-finder 가 `active` 저장 → 트리거 `bp_lifecycle_sync_ins/_upd`(`status='active' AND project_id IS NULL`) → ① projects 생성 ② BP에 project_id 역기입 ③ accepted+연결된 코치를 project_members 등록. 종료/취소 → `bp_status_propagate_upd` 가 projects.status(closed/archived) 동기화
...
| `projects` / `project_members` | 코칭 engagement + 멤버(가시성 키). `projects.required_kpis`(jsonb) = 프로젝트별 필수 KPI(ADR-022) |
...
- **프론트**: 빌드 없는 단일 `public/index.html`(~10,880줄 바닐라 JS) + `public/field-defs.js`(필드 SoT, ADR-020). `switchView`+해시 라우팅. XSS 방어 = `escHtml`/`escAttr` 전역. i18n ko/en/ja.
...
- **단일 index.html ~10,880줄** — 모듈화 미착수(대형, ADR 게이트)
...
> 상세 백로그: [docs/history/AUDIT-2026-06-01-verification.md](history/AUDIT-2026-06-01-verification.md).
